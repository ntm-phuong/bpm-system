...
export const Sidebar = ({
	...
	return (
		<aside className={`${styles.sidebar} ${isCollapsed ? styles.collapsed : ''}`} aria-label="BPM sidebar">
          ...
          <div className={styles.logoWrapper}>
			...
			<nav className={styles.navigation}>
				...
				{navigationGroups.map((group) => {
					const processItems: INavigationItem[] = group.key === 'createProcess'
					...
					return (
						<section className={styles.group} key={group.key}>
							...
							>
								...
								{!group.isStandalone && (
									<Icon className={styles.toggleIcon} iconName={isExpanded ? 'ChevronDown' : 'ChevronRight'} />
								)}
							</button>
							...
							{isExpanded && !isCollapsed && !group.isStandalone && (
								<div className={styles.groupItems}>
									{group.key === 'createProcess' && loading && (
										<span className={styles.groupEmpty}>Dang tai quy trinh...</span>
									)}
									{group.key === 'createProcess' && !loading && processItems.length === 0 && (
										<span className={styles.groupEmpty}>Khong co quy trinh active</span>
									)}
										...
										<button
											...
											onClick={() => {
												...
												if (group.key === 'createProcess') {
													const processId = Number(item.key.replace('process-', ''));
													const selectedMenuItem = menuItems.find((process) => process.processId === processId);
													if (Number.isNaN(processId) || !selectedMenuItem) {
														setSelectedProcess(null, null);
														return;
													}

													setSelectedProcess(processId, selectedMenuItem.processCode);
												} else {
													setSelectedProcess(null, null);
												}
											}}
